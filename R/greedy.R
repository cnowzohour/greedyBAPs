  ...
  if (method == "eigenvalue") {
    ...
    if (! is.null(data)) {
      ...
      score <- -0.5 * ( length(comp$cnodes) * log(2*pi) + log(det(res$Shat)/prod(diag(res$Shat[indices,indices,drop=FALSE]))) +
        ...
        edge.penalty * log(n)/n * n.edges
    } else {
      # Population version
      if (n == Inf) {
        penalty <- 0
      } else {
        penalty <- edge.penalty * log(n)/n * n.edges
      }
    ...
    score <- -0.5 * ( length(comp$cnodes) * log(2*pi) + log(det(res$Shat)/prod(diag(res$Shat[indices,indices,drop=FALSE]))) +
                        (n-1)/n*(sum(diag(solve(res$Shat) %*% covMat[nodes,nodes,drop=FALSE]))-length(pa)) ) - edge.penalty * log(n)/n * n.edges
  }
